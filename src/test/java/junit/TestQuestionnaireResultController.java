...
class TestQuestionnaireResultController {
    ...
    void testGenerateQuery() {
        String query1 = "SELECT catId as id, name, imgSrc, dayOfBirth, monthOfBirth, yearOfBirth, gender, 1 AS type, shelter FROM Cats JOIN Compatibility ON catId = petId WHERE gender = '1' AND sleepOutside = '1' AND firstExperience = 1 AND (year(now()) - yearOfBirth <= 1) AND (hoursAlone = 1 OR hoursAlone = 2)";
